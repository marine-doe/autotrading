...
def get_daily_price_avg(code):
    ...
    for response in response_list:
        ...
        if (datetime.datetime.strptime(date_to, "%Y%m%d") - datetime.datetime.strptime(response['stck_bsop_date'], "%Y%m%d")) <= datetime.timedelta(days=6):
            ...
            fcnt+=1
    return five_days_avg / fcnt, twenty_days_avg / tcnt
...
try:
    ...
    # 관심종목 현재가
    stocks = {}  # ex : 삼성전자 : 81200
    for key in symbol_list.keys():  # db에 관심종목이 없으면 새로 넣긔
        current_price = get_current_price(key)
        stocks[symbol_list[key]] = current_price
        new_document = {
            "code": key,
            "name": symbol_list[key],
            "value": 0
        }
    ...
    # 보유 현금
    cash = collection.find_one({"code": "-1"})["value"]  # 보유 현금 조회
    send_message(f"***주문 가능 현금 잔고: {cash}원***")
    ...
    message = "\n=====주식 보유 잔고=====\n"
    for stock, price in stocks.items():
        amount = collection.find_one({"name": stock})["value"]
        result = price * amount
        message += f"{stock} : {result}\n"
        total += result
    message += "======================"
    send_message(message)

    #총 자산
    send_message(f"***총 자산: {cash + total}***")
    ...
    buy_amount = cash * buy_percent  # 종목별 주문 금액 계산
    soldout = False
